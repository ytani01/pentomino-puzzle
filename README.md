...
| 一手戻す / おまかせ / ヒント表示 / やり直し / 音の ON・OFF / タイトルへ | 画面上端のアイコンのボタン。マウスを載せると、タッチ端末では押したときに、何のボタンかが出る |
...
押すと）名前が出る。速さは **ゆっくり**・**速い**・**最速** から選べる
（右向きの三角が 1・2・3 個）。どの速さでも 1 手ずつ進むので、置いては外す
様子を目で追える。解を見つけると止まるので、**次の解を探す**（虫眼鏡）で続きを見られる。試した手数と見つけた
解の数を画面に出す。音の ON・OFF と **タイトルへ** も同じ画面にある。
