...
| `src/ui.js` | ボタンと枠の組み立て。5 つのシーンが同じ見た目を使うため。HUD のボタンの説明（ツールチップ。TODO-042）。8×8 の穴に嵌めるアクリルの板 `drawAcrylic()`（本編と記録の完成形で共通。TODO-051）。縮小した盤 `drawMiniBoard()`（記録の完成形とタイトルの動く盤で共通。TODO-087） |
