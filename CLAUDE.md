...
概要は [README.md](README.md) を見る。以下はコードを読んでも分からない決めごと。
...
見た目の確認は `screenshot` スキルを使う（Playwright MCP で headless の
Chromium を動かして撮る。TODO-034）。
...
| `src/logic.js` | 向きの生成・正規化、配置判定、盤面の更新、対称な解の代表形（TODO-012）、時間の整形（純関数）、デモ用に 1 手ずつ返す探索 `solveSteps()`（TODO-040）、ヒント表示で自動で置く手 `forcedPlacements()`（TODO-044） |
...
| `src/scenes/demo.js` | コンピューターが探索して解を見つける様子を見せるデモ。`GameScene` を継承し盤・トレイの描画を使い回す。記録・遊びかけ・見つけた解には何も残さない（TODO-040） |
