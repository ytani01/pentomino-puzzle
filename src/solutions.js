...
/**
 * 盤ごとのデータの読み込み。**動的 import にする**のは、6×10 の 139KB を
 * その盤を選ぶまで読まないため（タイトル画面と 8×8 では読まない）。
 *
 ...
 *
 * 変換を `boardSymmetries()` から取るのは、盤の形を保つものだけに絞るため
 * （6×10 を 90° 回すと 10×6 になって盤に載らない）。対称な解は写しても
 ...
 * 持たせない決まりのため（盤や色の選択と同じ）。2 つのシーンがほぼ同時に
 ...
 * 要らない。TODO-017）。終盤に候補が 1 つしか残っていなければ同じ手になるが、
 * それは他の手が無いということ。
 *
