 ...
 * 盤面・ピース・色・レイアウトの定数。
 *
 * ボードの座標系を全ファイルが共有するので、数値と色はここへ集める。
 * マスの大きさを 1 か所直せば、ボード・トレイ・テクスチャがまとめて追従する。
 */

/**
 * 選べるボード。穴を「中央 2×2」と書かずに矩形で持つのは、`logic.js` の
 * `boardCells()` をボードの形に依存させないため。穴の無いボードは `hole: null`。
 *
 * どちらも置けるマスは 60 で、12 種がちょうど収まる（TODO-009）。
 * `note` はタイトルの遊び方に出す但し書き。ボードを足しても文面を直さずに済むよう
 * ボードの側に持たせる。
 *
 * `storageKey` をボードごとに分けるのは、難しさの違うボードの最短時間を混ぜないため。
 * 8×8 だけ接尾辞が無いのは、ボードが 1 つだった頃の記録を引き継ぐため。
 *
 ...
 *
 * `progressKey` をボードごとに分けるのは、8×8 を遊びかけたまま 6×10 を始めても
 * 両方の続きを残すため（TODO-030）。
 *
...
/**
 * ボード 1 つぶんに残すクリア記録の件数（TODO-008）。あふれたら古いものから捨てる。
 *
...
/**
 * 選んだボードを覚えておく `game.registry` のキー。タイトルが書き、他のシーンが
 * 読む（TODO-009）。記録画面の「この回を続ける」も、見ているボードで本編を
 * 始めるために書く（TODO-073）。
 *
 * `scene.start()` の引数で回さないのは、タイトル↔本編↔クリアの行き来が
 * 6 通りあり、どこかで渡し漏れると黙って既定のボードに戻るため。
 */
export const BOARD_REGISTRY_KEY = 'board';

/** ボードのマスのうち、ピースを置ける数。ペントミノ 12 種 × 5 マスと一致する。 */
...
/** 選んだ色の組の保存先。ボードごとの記録（`BOARDS[key].storageKey`）とは別に 1 つだけ。 */
export const PALETTE_STORAGE_KEY = 'pentomino-puzzle/palette';

/** ボードのマスの状態。空きは `null` で表す。 */
export const HOLE = '#';
...
/**
 * トレイのスロットを、ボードに近い棚から詰めて並べる（TODO-053）。
 *
 * 等分にしないのは、トレイの枠が広いほどピースどうしが離れ、ボードまで指を
 * 動かす距離が延びるため。スロットの一辺はピースの長い辺で決める（回しても
 * 変わらないので、どの向きでもはみ出さない）。大きい順に並べるのは、
 * 棚の奥行き（その棚で一番大きいスロット）を無駄にしないため。
 *
 * 返す座標は、沿う向き `along` が 0〜`length`、奥行き `depth` がボードの側の端から。
 * 画面の座標へは、ボードの位置が決まってから `makeLayout()` が直す。
 */
 ...
 * 画面の配置を作る。
 *
 * 関数にするのは、ボード（8×8・6×10）と画面の向き（横・縦）の組み合わせが
 * 4 通りあり、手で並べた数値では食い違いを防げないため（TODO-011）。
 * マスの大きさも、ボードに使える幅と高さの小さいほうから決める。
 *
 * 場所は**トレイを先に決めて、余りを全部ボードに回す**。トレイはピースが
 * 掴めればよく、大きく見せたいのはボードのほうだから。
 *
 * - 横画面（ボードが左・トレイが右）… トレイは高さいっぱいなので、スロットを
 *   縦に詰めて棚にし、棚の幅の合計をボードの取り分から差し引く
 * - 縦画面（ボードが上・トレイが下）… トレイは幅いっぱいなので、スロットを
 *   横に詰めて棚にし、棚の高さの合計をボードの取り分から差し引く
 *
 * スロットは `packTray()` が詰めて並べ、ボードの側の端から置く（TODO-053）。
 */
export function makeLayout({
  ...
  // 横画面は高さが足りないので 1 行に詰める。その分だけボードとトレイが縮む。
  ...
  // 少ないと、デモのボードとトレイだけが下へずれるため（`DEMO_LAYOUTS`）。
  ...
  // 交わる向き（横画面なら上下、縦画面なら左右）は中央へ寄せる。ボードが
  ...
  // スロットは枠のボード側の端から詰めるので、余りはボードから遠い側に出る。
...
/**
 * ボードに依らない画面の寸法。向きごとに 1 組（`SCREENS.portrait` など）。
 * Phaser の内部解像度（`main.js`）と、ボードを持たないタイトル・クリア・記録の画面が読む。
 */
export const SCREENS = byOrientation((portrait) => ({ portrait, margin: MARGIN, ...screenSize(portrait) }));

/**
 * ボードごとの配置。**向きとボードの組み合わせぶんを起動時にまとめて作る**（TODO-009・TODO-095）。
 * 引くときは `LAYOUTS[向き][ボード]`。
 *
 * 選ぶたびに作らないのは、書き換わる状態をモジュールに持たせないため。
 * 中身は画面の向きとボードの大きさだけで決まるので、先に作っても同じになる。
 */
 ...
 * ただしタイトル行の下に探し方の一文（`note`）を足すので、ボードとトレイは
 * その 1 行ぶん本編より下がり、横画面ではマスも小さくなる。そのマスの
 * テクスチャは Boot が焼く（TODO-089）。引くときは `DEMO_LAYOUTS[向き][ボード]`。
 */
 ...
 *
 * 画像を使わずにガラスらしく見せるため、**地を半透明にしてボードのマスを透かし**、
 * 内側の明るい縁と斜めの光の筋を重ねる。筋の位置はマスの一辺に対する割合
 * （ボードで 64px、トレイで 20px と大きさが違うため）。
 *
...
export const GLASS = {
  ...
  // 5 マスが 1 個に見えず、ボードが一面のタイル貼りに見える。
  gridColor: 0x000000,
  gridAlpha: 0.22,
...
/**
 * ボードの空きマスの下地（TODO-094）。`GLASS` と同じ作りで、枠の地を透かす
 ...
 *
 * 値はボードのマス 1 個ぶんを 1 とした割合。ピースはボードのマスの座標系で描き、
...
/**
 * タイトルのボード・色の選択肢に描く図（`src/icons.js`。TODO-046・TODO-087）。
 *
 * - `boardCell` / `boardGap` … ボードの図の 1 マスと、マスの間の隙間。8 マスで
 ...
 *   （TODO-092）。`NEON.glow` はボードの 1 マス（`OUTLINE.width` の説明にある
 ...
 *   （TODO-081）。無いと、広い空きが残るボードでは気づくまでに百手以上重ねるため。
 *   8 は利用者が決めた
 ...
 * 完成したボードと「完成」の文字を、表示で隠す前に見せる。
 */
export const CLEAR_DELAY_MS = 700;

/**
 * タイトルの動くボード（TODO-087）。デモのランダムな探索を小さなボードで回す飾り。
 *
 * - `intervalMs` … 1 手の間隔。デモの「速い」（`DEMO.speeds.fast`）と同じ
 * - `pauseMs` … 解けてから空のボードで探し直すまで。デモ（`DEMO.pauseMs`）より
