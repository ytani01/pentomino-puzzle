...
**残っている項目: TODO-061〜066。** これまでに 60 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。
**新規項目の番号は `TODO-067` から。**

着手する順番の案（TODO-061〜065）: 062 → 061 → 063 → 064 → 065
...
- [ ] `solveStepsRandom()`: 5 マスの閉じた空きがあり、その形のピースがトレイに残っていれば、ほかの手より先にそこへ置く
- [ ] `tests.html` にテストを足す
- [ ] `docs/UsersGuide.md`・`docs/developer.md` のランダムの説明を直す

人は、ちょうどピース 1 つ分の穴を見れば、それに合うピースを探して埋める。

- 合う置き方が控え（`failed`）に入っていれば選ばず、今までどおりに選ぶ
- 合うピースが無い 5 マスの空きは、今までどおり行き詰まってから戻す（利用者が決めた）
- TODO-061・062 と同じ関数を触るので、同時には進めない
