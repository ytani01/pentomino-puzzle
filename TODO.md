...
**残っている項目: TODO-061〜065・067・068。** これまでに 61 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。
**新規項目の番号は `TODO-069` から。**

着手する順番の案（TODO-061〜065・067・068）: 062 → 067 → 068 → 061 → 063 → 064 → 065

- 062 を 061 より先にする。重みに距離を足すだけで、決めることが無い
- 063 は 060〜062 のあとにする。置き方が変わると詰まり方も変わるため
- 064 と 065 は探索を変えない見せ方なので最後にする。065 の回す時間は
  064 の待ち時間と重なるので、064 を先にする
- 067・068 は 060・066 と同じ「置いた直後に外す」の延長で決めることが無いので、062 の次にする
...
- [ ] `solveStepsRandom()`: 置いた直後に、閉じた 5 マスの空きの形がもう盤に置いたピースと同じなら、その手をその場で外す
- [ ] `tests.html` にテストを足す
- [ ] `docs/UsersGuide.md`・`docs/developer.md` のランダムの説明を直す

ピースは 1 種 1 つなので、置き済みのピースの形の穴は埋められず、必ず解なしになる。
人は置いた瞬間にこれに気づく。外すのは穴を作った直前の 1 手だけで、控え `failed` に
入れて選び直さない（TODO-060 の小さな空きと同じ扱い。利用者が決めた）。
5 マスの空きは必ずどれかのピースの形なので、残りのピースの形なら TODO-066 で埋め、
置き済みのピースの形ならこの項目で外すことになる。
...
- [ ] `solveStepsRandom()`: 置いた直後に、閉じた空きの大きさが 5 の倍数でなければ、その手をその場で外す（TODO-060 の「ピースより小さい空き」を広げる）
- [ ] `tests.html` のテストを直す・足す
- [ ] `docs/UsersGuide.md`・`docs/developer.md` のランダムの説明を直す

TODO-060 では、すぐ気づくのは 1〜4 マスの空きだけにして、7 や 12 マスのような
大きい空きは行き詰まってから戻すと決めていた。利用者がこれを改め、5 の倍数でない
空きは大きさによらずその場で外すことにした。外すのは直前の 1 手で、控え `failed` に
入れて選び直さない（TODO-067 と同じ扱い）。
TODO-060 の既存テストのうち「大きい空きはその場で外さない」を確かめているものは
前提が崩れるので直す。
