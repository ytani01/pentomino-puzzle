...
`src/config.js` の `makeLayout()` が返す配置（以下 `LAYOUT` と呼ぶ。実際に
export してあるのは、盤ごとの `LAYOUT` を集めた `LAYOUTS`）のキーと、画面の
部位の対応。やり取りでもソースでもこの呼び名を使う。
...
| **おまかせ**（auto） | ボタンの 1 つ。**押すたびに 1 個だけ**、全解のデータから選んだ手を盤へ置く（TODO-022）。使うと最短時間には入らず、記録には印が残る（TODO-024） |
| **ヒント表示**（hint） | ボタンの 1 つ。入にすると、置く・外すたびに残りのピースで最後まで置けるかを調べて HUD に出す（TODO-013）。解けるときは、切り離された 5 マスの空きが残りのピースと同じ形なら自動で置く（`forcedPlacements()`。履歴を積まないので、一手戻すと直前の手とまとめて戻る。TODO-044）。こちらも使うと最短時間には入らない |
| **HUD**（hud） | 上端の帯。1 段目に経過時間・残り数・解の有無、その下にボタンが並ぶ。ボタンは 6 個で、横画面は 1 段、縦画面は 3 個ずつ 2 段に折り返す（TODO-026 より前は、横画面だけ文字と同じ段に並べていた）。ボタンは文字でなくアイコン（`src/icons.js`）で、何のボタンかは**説明**に出す（TODO-042） |
...
| 呼び名 | 何を指すか |
|---|---|
| **一覧**（list） | クリアした回を新しい順に並べた行。選んだ行を強調し、1 頁ぶんずつ**頁送り**する。日時と時間は行の左端から、**印**は右端から（TODO-027） |
| **印**（mark） | 行の右端に出る「おまかせ」「ヒント」（TODO-024・TODO-027・TODO-028）。その回が何に頼って解かれたかを示す。両方なら「おまかせ・ヒント」 |
| **完成形**（mini） | 選んだ回の完成した盤面を縮小して描いたもの。塗りとピースの境目だけで描く |
| **達成度**（achieve） | 「8×8 … 65 解中 12 解」の 1 行（TODO-022）。分母が盤で違うので**盤の名前を頭に付ける** |

デモの画面（`src/scenes/demo.js`。TODO-040）で使う呼び名:
...
| 解ける / 解なし | HUD の 1 段目の右端に出す、置いた直後の盤が解につながるか。本編のヒント表示と同じ札（`createHintBadge()`。TODO-045）で、全解のデータ（`hasSolution`）で決める。解なしならその手はすぐ外す（TODO-043） |
...
盤ごとの解を**全部あらかじめ数え上げて持っている**（TODO-022）。おまかせも
...
`.claude/agents/*.md` に置いてある（TODO-037）。1 回きりの役割は今までどおり
