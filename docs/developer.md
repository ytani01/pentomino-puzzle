...
| **タイトル行** | 本編・記録・デモの最上段の「PENTOMINO PUZZLE」。押すとタイトルへ戻る（本編は HUD の「タイトルへ」と同じ確認を挟む）。右横に小さくバージョンを添える（本編と記録は右下の `createVersionText()` を呼ばず、バージョンはここだけに出す。TODO-093）。デモではその下に探し方が 2 種類あることを一文で出し、そのぶん盤とトレイが本編より 1 行下がる（横画面ではマスも小さくなる）。`src/ui.js` の `createTitleBar()` が作る（TODO-089） |
