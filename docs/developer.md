...
| 解ける / 解なし | HUD の 1 段目の右端に出す、置いた直後の盤が解につながるか。本編のヒント表示と同じ札（`createHintBadge()`）で、全解のデータ（`hasSolution`）で決める。深さ優先は解なしならその手はすぐ外す。ランダムは置ける手が尽きるまで外さず、尽きたら解けるに戻るまで外す（TODO-059） |
