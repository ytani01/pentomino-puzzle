...
| **探し方**（深さ優先・ランダム） | 探索の順（TODO-050・TODO-057）。深さ優先は `solveSteps()`、ランダムは `solveStepsRandom()`。切り替えると空の盤から探し直す。ランダムはまずピースを、置ける手が一番少ない空きマス「狭い所」（`countCellMoves()`）を覆えるものが選ばれやすいように抽選し（`DEMO.randomTightWeight`。TODO-061）、覆えるなら置き方も狭い所を覆う手に絞る。置き方の抽選は、盤の端・置いたピースの隣に接するほど（`touchWeight()`）、直前に置いた手に近いほど（`moveDistance()`。TODO-062）重みが大きくなる。同じ深さ（盤に残るピースの数）への「詰まり」（置ける手が尽きるか、解の無い盤面で `DEMO.randomDeadLimit` 手置いて戻る行き詰まりの一続きのみ。TODO-081。置いた直後にその場で外す手は数えない）が `DEMO.randomCollapseAfter` 回続いたら、続けて `DEMO.randomCollapseMoves` 手をまとめて外す（人は同じ所で詰まり続けると大きく崩すため。TODO-063）。崩した手も 1 手ずつ通常どおり控えに入る。`ゆっくり`・`速い` では、盤へ滑らせる前に
