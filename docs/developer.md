...
| **次の解を探す** | 解を見つけて止まっているときだけ押せるボタン。押すと 10 秒（`DEMO.pauseMs`）待たずに次の解へ進む（深さ優先は盤を片づけて最初から、幅優先は続きから） |
| **探し方**（深さ優先・幅優先） | 探索の順（TODO-050）。深さ優先は `solveSteps()`、幅優先は `solveStepsBreadth()`。切り替えると空の盤から探し直す。幅優先で次の途中の盤面へ移るときの外す・置き直す手を**置き直し**（`replay`）と呼ぶ |
| 試した手 / 見つけた解 | HUD の 1 段目に出す数。試した手はピースを置いた回数（「解なし」で外した置き方も含む。外した回数と置き直しは数えない）、見つけた解は解けて止まった回数 |
