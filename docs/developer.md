...
| **探し方**（深さ優先・ランダム） | 探索の順（TODO-050・TODO-057）。深さ優先は `solveSteps()`、ランダムは `solveStepsRandom()`。切り替えると空の盤から探し直す。ランダムはまずピースを、置ける手が一番少ない空きマス「狭い所」（`countCellMoves()`）を覆えるものが選ばれやすいように抽選し（`DEMO.randomTightWeight`。TODO-061）、覆えるなら置き方も狭い所を覆う手に絞る。置き方の抽選は、盤の端・置いたピースの隣に接するほど（`touchWeight()`）、直前に置いた手に近いほど（`moveDistance()`。TODO-062）重みが大きくなる |
