...
盤に何も置いていなければ確認を挟まずに移り、前の遊びかけも残す（TODO-094）。
Game の「やり直し」も同じ確認パネル（`showConfirm()`）を挟み、`scene.restart()` で作り直す
（盤が空なら確認なし）。
...
| **ヒント表示**（hint） | ボタンの 1 つ。入にすると、置く・外すたびに残りのピースで最後まで置けるかを調べて HUD に出す。解けるときは、切り離された 5 マスの空きが残りのピースと同じ形なら自動で置く（`forcedPlacements()`。履歴を積まないので、一手戻すと直前の手とまとめて戻る）。自動で置くのはピースを置いたとき（おまかせを含む）と入にしたときだけで、外す・一手戻す・向きを変えるときは置かない（外したピースが埋め戻されて完成した盤を変えられなくなるため。TODO-072）。こちらは使っても最短時間に入る（記録には印が残る。TODO-088） |
| **タイトル行** | 本編・記録・デモの最上段の「PENTOMINO PUZZLE」。押すとタイトルへ戻る（本編は HUD の「タイトルへ」と同じ確認を挟む。盤が空なら挟まない）。右横に小さくバージョンを添える（本編・記録・デモは右下の `createVersionText()` を呼ばず、バージョンはここだけに出す。TODO-093）。デモではその下に探し方が 2 種類あることを一文で出し、そのぶん盤とトレイが本編より 1 行下がる（横画面ではマスも小さくなる）。`src/ui.js` の `createTitleBar()` が作る（TODO-089） |
| **HUD**（hud） | タイトル行の下の帯。1 段目に経過時間・残り数・解の有無（盤が完成しているときは、同じ位置に「新しい解」「記録を更新」「記録済み」の知らせ）、その下にボタンが並ぶ。ボタンは 6 個（デモは 7 個）で、一番左がタイトルへ。どちらの向きでも 1 段に並ぶ（入りきらなければ折り返す）。ボタンは文字でなくアイコン（`src/icons.js`）で、何のボタンかは**説明**に出す |
| **操作の概要**（help） | 本編の下端、トレイとメッセージの行の間に出す操作の説明。文言はタイトルの遊び方と同じ `src/ui.js` の `HOW_TO_OPERATE`。縦画面は 3 行、横画面は 1 行に詰めた `HOW_TO_OPERATE_SHORT`。デモには出さない（`makeLayout()` の `help`。TODO-094） |
...
| **ゴミ箱**（trash） | 「全部選ぶ」と同じ行の右端（一覧の右端に揃える）のボタン（TODO-093）。チェックが 1 件以上のときだけ押せ、画面内で件数を確かめてから消す。その盤の記録が 1 件も残らなければ、達成度とおまかせの番号も消す |
...
| **探し方**（深さ優先・ランダム） | 探索の順（TODO-050・TODO-057）。深さ優先は `solveSteps()`、ランダムは `solveStepsRandom()`。切り替えると空の盤から探し直す。ランダムは、覆えるピースが 1 種類しか無い空きマスがあれば、そのピースでそのマスを覆う手に絞る（`singlePieceCell()`。TODO-082。5 マスの穴を埋める手（`forcedPlacements()`）の次に優先する。控えに入れた手は除いて数えるので、別のピースを試してだめだったマスも当たる）。無ければまずピースを、置ける手が一番少ない空きマス「狭い所」（`countCellMoves()`）を覆えるものが選ばれやすいように抽選し（`DEMO.randomTightWeight`。TODO-061）、覆えるなら置き方も狭い所を覆う手に絞る。置き方の抽選は、盤の端・置いたピースの隣に接するほど（`touchWeight()`）、直前に置いた手に近いほど（`moveDistance()`。TODO-062）重みが大きくなる。同じ深さ（盤に残るピースの数）への「詰まり」（置ける手が尽きるか、解の無い盤面で `DEMO.randomDeadLimit` 手置いて戻る行き詰まりの一続きのみ。TODO-081。置いた直後にその場で外す手は数えない）が `DEMO.randomCollapseAfter` 回続いたら、続けて `DEMO.randomCollapseMoves` 手をまとめて外す（人は同じ所で詰まり続けると大きく崩すため。TODO-063）。崩した手も 1 手ずつ通常どおり控えに入る。`ゆっくり`・`速い` では、盤へ滑らせる前に
トレイでの今の向きから置く向きまで、`orientationSteps()`（`logic.js`）が返す
最短の回転・裏返しで 1 段ずつ回して見せる（`playTurns()`。TODO-065） |
| 試した手 / 見つけた解 | HUD の 1 段目に出す数。試した手はピースを置いた回数（外した置き方も含む。外した回数は数えない）、見つけた解は解けて止まった回数 |
| 解ける / 解なし | HUD の 1 段目の右端に出す、置いた直後の盤が解につながるか。本編のヒント表示と同じ札（`createHintBadge()`）で、全解のデータ（`hasSolution`）で決める。深さ優先は解なしならその手はすぐ外す。ランダムは置ける手が尽きるまで外さず、尽きたら解けるに戻るまで外す（TODO-059）。解なしの盤面で置いた手（その場で外した手も含む）が `DEMO.randomDeadLimit` 手に達したときも、尽きるのを待たずに同じく戻る（TODO-081）。ただしランダムでも、5 の倍数でない大きさの閉じた空きができたとき、5 マスの穴に合う残りのピースで埋めた直後に解なしになったとき、残りのどのピースでも覆えない空きマスができたとき（`hasUncoverableCell()`。置き済みのピースと同じ形の 5 マスの空きもこれに当たる）は、行き詰まりを待たずその場で外す（2 つ目はその前に置いた手もまとめて外す。TODO-060・TODO-066・TODO-067・TODO-068・TODO-077） |
...
| 用途 | キー（8×8 の場合） | 値の形 |
|---|---|---|
| 最短時間 | `pentomino-puzzle/best-ms` | ミリ秒の数値を文字列にしたもの。読むときは履歴のうちおまかせを使っていない回とも比べ、短い方を最短とする（`loadBest()`。TODO-088） |
...
おまかせの印（`a`）が無いほうを先に立て、印の有無が同じなら `ms` の短いほうをよいと
する（ヒント表示の印 `h` は見ない。最短時間の区切り `shouldRecordBest()` と同じ。TODO-088）。
...
| タイミング | 呼ぶもの |
|---|---|
| 盤の状態が変わるたび／シーンを離れるとき | `saveProgress()`（1 個も置いていなければ代わりに `clearProgress()`。ただし消すのは、そのシーンで盤に置いたか続きから始めたときだけ。TODO-094） |
| おまかせで解を出したとき | `addAuto()` |
| 完成したとき（続けて作った解も） | `recordCompletion()`（中で `saveBest()`（おまかせを使っていないときだけ）・`recordClear()`・`addFound()`）・`saveProgress()` |
| やり直したとき（何も置かずに押したときを除く）・クリアの表示で「もう一度」 | `clearProgress()` |
...
flowchart LR
  subgraph Game
    G1["置く・外す・<br>シーンを離れる"] --> P["saveProgress()<br>（空なら clearProgress()。<br>何も置いていないシーンでは消さない）"]
    G2["おまかせ"] --> A["addAuto()"]
    G3["やり直し"] --> C["clearProgress()"]
    G4["完成"] --> RC["recordCompletion()"]
    RC --> B["saveBest()<br>（おまかせを使っていないときだけ）"]
...
その盤の記録が 1 件も残らないときは、`found` と `auto` を丸ごと消す。
履歴（50 件まで）からあふれた番号や、履歴に無いおまかせの番号まで消さないと、
一覧が空なのに達成度が 0 にならないため。
