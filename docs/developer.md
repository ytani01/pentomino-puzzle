...
- ボードごとの全解をあらかじめデータとして持つ（`src/data/`）ので、おまかせと
...
src/
  ...
  ui.js               ボタンと枠（5 つのシーンで共通）、穴のアクリルの板、縮小したボード
  icons.js            HUD のボタンのアイコンと、タイトルのボード・色の選択肢の図
  scenes/
    boot.js           マス目テクスチャの生成
    title.js          タイトル（動くボードを含む）
    game.js           本編
    clear.js          クリア表示
    records.js        クリア記録の一覧
...
Demo へ進む。Demo は URL のボードと探し方を使い、`registry` のボードは書き換えない。
...
ボードに何も置いていなければ確認を挟まずに移り、前の遊びかけも残す（TODO-094）。
...
| シーン | 持ち越すもの |
|---|---|
| Title | なし（ボード・色は `registry` にある）。動くボードは始め直す |
...
| Records | 見ているボード・選んでいる回・チェック・開いている確認。1 頁の行数が向きで違うので、頁は見えていた回（選んでいる回か頁の先頭）が載る頁に合わせ直す |
| Demo | 探索の generator（続きから進む）・ボードのピース・探し方・速さ・数えた手と解・メッセージ。向きを回して見せている途中なら、先に仕上げてから控える |
...
| キー | 中身 |
|---|---|
| `BOARD_REGISTRY_KEY`（`'board'`） | 選んでいるボード（`'8x8'` / `'6x10'`） |
...
| `solutions/<ボード>` | 読み込み済みの全解のデータ。`solutions.js` の `solutionsRegistryKey()` が `SOLUTIONS_REGISTRY_PREFIX`（`'solutions/'`）からボードのキーを添えて作る |

`BOARD_REGISTRY_KEY` と `PALETTE_REGISTRY_KEY` は Boot が起動時に既定値で
一度書き込み、Title で選び直すたびに書き換える。`BOARD_REGISTRY_KEY` は
Records の「この回を続ける」でも、記録の画面で見ているボードへ書き換える
（本編は `registry` のボードで始まるため。TODO-073）。`registry` は起動のたびに
初期化されるので、ボード・色の選択はページを読み込み直すと既定へ戻る
...
`LAYOUT` と呼ぶ。実際に export してあるのは、向きとボードごとの `LAYOUT` を集めた
`LAYOUTS`。引くときは `LAYOUTS[向き][ボード]`）。その `LAYOUT` のキーと画面の部位の対応を、以下にまとめる。
やり取りでもソースでもこの呼び名を使う。
...
| 呼び名 | 何を指すか |
|---|---|
| **ボード**（board） | 左側（縦画面では上側）のマス目。タイトルで 8×8（中央 2×2 が**穴**（hole）で置けない）と 6×10（穴なし）から選ぶ |
| **トレイ**（tray） | 右側にある、**まだボードに置いていないピースの置き場**。「盆」「受け皿」の意味 |
| **スロット**（slot） | トレイの中の、**ピース 1 種ぶんの正方形の区画**。「差し込み口」の意味。ペントミノが 12 種なので 1 種につき 1 区画、置き場所が決まっている。一辺はピースの長い辺に合わせ（`I` は 5 マス、`L`・`N`・`Y` は 4 マス、他は 3 マス）、大きい順にボードの側から詰めて並べる（`makeLayout()` の `tray.slots`） |
| **パネル**（panel） | ボードやトレイの外側の枠（`boardPanel` / `trayPanel`）。`src/ui.js` の `createPanel` が描く |
| **ゴースト**（ghost） | ドラッグ中、置ける場所に出る薄い影 |
| **おまかせ**（auto） | ボタンの 1 つ。**押すたびに 1 個だけ**、全解のデータから選んだ手をボードへ置く。使うと最短時間には入らず、記録には印が残る |
| **ヒント表示**（hint） | ボタンの 1 つ。入にすると、置く・外すたびに残りのピースで最後まで置けるかを調べて HUD に出す。解けるときは、切り離された 5 マスの空きが残りのピースと同じ形なら自動で置く（`forcedPlacements()`。履歴を積まないので、一手戻すと直前の手とまとめて戻る）。自動で置くのはピースを置いたとき（おまかせを含む）と入にしたときだけで、外す・一手戻す・向きを変えるときは置かない（外したピースが埋め戻されて完成したボードを変えられなくなるため。TODO-072）。こちらは使っても最短時間に入る（記録には印が残る。TODO-088） |
| **タイトル行** | 本編・記録・デモの最上段の「PENTOMINO PUZZLE」。押すとタイトルへ戻る（本編は HUD の「タイトルへ」と同じ確認を挟む。ボードが空なら挟まない）。右横に小さくバージョンを添える（本編・記録・デモは右下の `createVersionText()` を呼ばず、バージョンはここだけに出す。TODO-093）。デモではその下に探し方が 2 種類あることを一文で出し、そのぶんボードとトレイが本編より 1 行下がる（横画面ではマスも小さくなる）。`src/ui.js` の `createTitleBar()` が作る（TODO-089） |
| **HUD**（hud） | タイトル行の下の帯。1 段目に経過時間・残り数・解の有無（ボードが完成しているときは、同じ位置に「新しい解」「記録を更新」「記録済み」の知らせ）、その下にボタンが並ぶ。ボタンは 6 個（デモは 7 個）で、一番左がタイトルへ。どちらの向きでも 1 段に並ぶ（入りきらなければ折り返す）。ボタンは文字でなくアイコン（`src/icons.js`）で、何のボタンかは**説明**に出す |
...
| **達成度**（achieve） | 「8×8 … 65 解中 12 解」の 1 行。分母がボードで違うので**ボードの名前を頭に付ける** |
| **チェック**（check） | 一覧の各行の左端の四角。押すと消す回として選ぶ／外す。行を押したとき（完成形を出す）とは別。頁を送っても残り、ボードを切り替えると消える |
| **全部選ぶ**（select all） | 一覧の上のチェック。見えていない頁の分も含め、そのボードの記録をすべて選ぶ／外す |
| **この回を続ける**（continue） | 完成形の下のボタン。選んだ回の完成形を並べたボードから本編を始める。時計とおまかせ・ヒントの印はその回のものを引き継ぐ。記録が無いときは出ず、全解のデータを読み込むまでは押せない |
| **ゴミ箱**（trash） | 「全部選ぶ」と同じ行の右端（一覧の右端に揃える）のボタン（TODO-093）。チェックが 1 件以上のときだけ押せ、画面内で件数を確かめてから消す。そのボードの記録が 1 件も残らなければ、達成度とおまかせの番号も消す |
...
| **次の解を探す** | 解を見つけて止まっているときだけ押せるボタン。押すと 10 秒（`DEMO.pauseMs`）待たずに次の解へ進む（ボードを片づけて最初から探し直す） |
| **探し方**（深さ優先・ランダム） | 探索の順（TODO-050・TODO-057）。深さ優先は `solveSteps()`、ランダムは `solveStepsRandom()`。切り替えると空のボードから探し直す。ランダムは、覆えるピースが 1 種類しか無い空きマスがあれば、そのピースでそのマスを覆う手に絞る（`singlePieceCell()`。TODO-082。5 マスの穴を埋める手（`forcedPlacements()`）の次に優先する。控えに入れた手は除いて数えるので、別のピースを試してだめだったマスも当たる）。無ければまずピースを、置ける手が一番少ない空きマス「狭い所」（`countCellMoves()`）を覆えるものが選ばれやすいように抽選し（`DEMO.randomTightWeight`。TODO-061）、覆えるなら置き方も狭い所を覆う手に絞る。置き方の抽選は、ボードの端・置いたピースの隣に接するほど（`touchWeight()`）、直前に置いた手に近いほど（`moveDistance()`。TODO-062）重みが大きくなる。同じ深さ（ボードに残るピースの数）への「詰まり」（置ける手が尽きるか、解の無い盤面で `DEMO.randomDeadLimit` 手置いて戻る行き詰まりの一続きのみ。TODO-081。置いた直後にその場で外す手は数えない）が `DEMO.randomCollapseAfter` 回続いたら、続けて `DEMO.randomCollapseMoves` 手をまとめて外す（人は同じ所で詰まり続けると大きく崩すため。TODO-063）。崩した手も 1 手ずつ通常どおり控えに入る。`ゆっくり`・`速い` では、ボードへ滑らせる前に
...
| 解ける / 解なし | HUD の 1 段目の右端に出す、置いた直後のボードが解につながるか。本編のヒント表示と同じ札（`createHintBadge()`）で、全解のデータ（`hasSolution`）で決める。深さ優先は解なしならその手はすぐ外す。ランダムは置ける手が尽きるまで外さず、尽きたら解けるに戻るまで外す（TODO-059）。解なしの盤面で置いた手（その場で外した手も含む）が `DEMO.randomDeadLimit` 手に達したときも、尽きるのを待たずに同じく戻る（TODO-081）。ただしランダムでも、5 の倍数でない大きさの閉じた空きができたとき、5 マスの穴に合う残りのピースで埋めた直後に解なしになったとき、残りのどのピースでも覆えない空きマスができたとき（`hasUncoverableCell()`。置き済みのピースと同じ形の 5 マスの空きもこれに当たる）は、行き詰まりを待たずその場で外す（2 つ目はその前に置いた手もまとめて外す。TODO-060・TODO-066・TODO-067・TODO-068・TODO-077） |
...
キーは `src/config.js` の `BOARDS` にボードごとに定義してある（`storage.js` は
キー文字列を直接書かない）。
...
| 色の組 | `pentomino-puzzle/palette` | 組の名前（ボードに依らないので 1 つだけ） |

6×10 は末尾が `/6x10` になる。ただし**最短時間の 8×8 だけ接尾辞が無い**——
ボードが 1 種類しかなかった頃の記録をそのまま引き継ぐためで、後から足したキーは
すべて接尾辞付きで揃えてある。
...
ボードの縦横は 1 件には持たせない。保存先のキーがボードごとに分かれているので、
どのボードの記録かはキーで決まる。
...
`'tray'`（未使用）か `'board'`（ボード上）。`solved` はそのプレーで完成させた解の
...
- **遊びかけは 1 か所でも変なら丸ごと捨てる**（`sanitizeProgress`）。12 個で
  ...
  `cells` が正当な向きのどれかであること（`orientations()` と照合）、ボードに
  置いてある分を順に置いていって重ならないこと（`canPlace()`）。12 個とも
  ボードに載った（完成した）盤面も通す。完成したあとも続けて遊び、ピースを
  入れ替えて別の解を作れるようにしたため
...
| タイミング | 呼ぶもの |
|---|---|
| ボードの状態が変わるたび／シーンを離れるとき | `saveProgress()`（1 個も置いていなければ代わりに `clearProgress()`。ただし消すのは、そのシーンでボードに置いたか続きから始めたときだけ。TODO-094） |
...
| チェックした記録を消したとき | `removeRecords()`（一部なら `removeHistoryMany()` + `removeFound()` + `removeAuto()`、そのボードの記録が残らなければ `removeHistoryMany()` + `clearFound()` + `clearAuto()`） |
...
そのボードの記録が 1 件も残らないときは、`found` と `auto` を丸ごと消す。
...
ボードごとの解を**全部あらかじめ数え上げて持っている**。おまかせも
...
- `src/config.js` の `PIECES`（ピースの形）と `BOARDS`（ボードの大きさ・穴）
...
**ボードを足したときは `tools/gen-solutions.mjs` を走らせ、`src/solutions.js` の
