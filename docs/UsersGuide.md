...
画面の下端には、ドラッグ・タップ・盤から外すの操作の概要がいつも出ている
...
（盤に何も置いていなければ確認なしで戻る。記録・デモの画面でも押せば戻り、そちらは常に確認なし）。
...
| 番号 | ボタン | 動き |
|---|---|---|
| 1 | タイトルへ | 確認を挟んでタイトルへ戻る。盤に何も置いていなければ確認なしで戻る |
| 2 | 一手戻す | 直前の 1 手を戻す |
| 3 | おまかせ | 押すたびに 1 個だけ、全解のデータから選んだ手を盤へ置く |
| 4 | ヒント表示 | 入にすると、置く・外すたびに残りのピースで最後まで置けるかを調べ、「解ける」「解なし」を HUD に出す |
| 5 | やり直し | 確認を挟んで、盤を空にして最初から（遊びかけも消える）。盤に何も置いていなければ確認なしですぐやり直す |
| 6 | 音 ON・OFF | 効果音の切り替え |
...
タイトルの右下にある **デモ** を押すと、選んでいる盤と色の組で、
空の盤からコンピューターが探索してピースを置いたり外したりしながら
解を見つける様子を眺められる。記録・遊びかけ・見つけた解には何も残らない。
置いた直後の盤が解につながるかを、本編と同じ「解ける／解なし」で HUD に出す（Ⓑ）。
...
同じところ（盤に残るピースの数）で何度も続くと、1 手ずつでなく数手まとめて
外し、大きく崩してやり直す
...
タイトルで選んでいる盤は変わらない。**タイトルへ** を押すと URL から
