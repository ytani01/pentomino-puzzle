...
[デモ](#デモ) のランダムな探し方が小さく動いている（飾りなので押しても何も起きない）。
...
**探し方** のボタンで、ランダム（線が絡まった点）と深さ優先（枝分かれする木）を
切り替えられる。既定はランダム。深さ優先は、左上の空いたマスから順に埋めていく。
...
| パラメータ | 値 |
|---|---|
| `demo` | 探し方。`random`（ランダム）か `depth`（深さ優先） |
| `board` | 盤。`8x8` か `6x10` |

省いたもの・知らない値はランダム・8×8 になる。色の組は前に選んだもの。
